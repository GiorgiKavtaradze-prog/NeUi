...
```bash
npx shadcn@latest add @neui/data-grid
```
...
**Already handled for you.** Every NeUI data-grid file you install (the primitive and the examples) already ships with `"use no memo"`, so installed NeUI code is safe out of the box.

**What you must do.** If you enable React Compiler, add `"use no memo"` to your own component that calls `useReactTable`, the one file NeUI cannot reach:
...
babel({
  ...
  exclude: ["**/components/neui/data-grid/**", "**/your-tables/**"],
})
```
...
} from "@/components/neui/data-grid"
```
